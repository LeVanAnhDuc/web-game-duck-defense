...
> **Phạm vi:** bản sao trong repo `duck-defense`. Bản canon nằm ở gốc workspace `web-game/`.
> **Trạng thái ở repo này:** 🟢 đã áp — xem §8 và ADR-0008
...
Mỗi ô: ✅ đã áp · 🟡 áp một phần, phần bỏ có lý do trong ADR · ⏸ hoãn có lý do trong ADR · ⬜ chưa · ➖ không áp dụng.

⚠️ Khảo sát đầu tiên của bảng này ĐẾM SAI test: nó chỉ glob `src/**/*.test.*` nên bỏ
sót thư mục `tests/` của `defense` (21 file) và `runner` (10 file). Đếm test thì đếm
cả repo, đừng đếm một thư mục.
...
| runner | Vite | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | `ui/`; 10 test ở `tests/` **gồm `tests/ui/flow.test.tsx`** — có lưới hành vi, KHÔNG bị R-07 chặn |
