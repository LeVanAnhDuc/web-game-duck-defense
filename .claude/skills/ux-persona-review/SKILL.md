...
| --- | --- |
| 2 · kiểm app đang chạy ở port | `curl -s -o /dev/null -w "%{http_code}" -L https://levananhduc.github.io/web-game-duck-defense/` phải trả `200`. Không phải `200` thì **dừng** và báo là bản deploy đang hỏng — đó tự nó đã là một phát hiện |
...
Cả hai định nghĩa ở `D:/DeleteByDuc/web-game/web-game-duck-defense/.claude/agents/`. Nếu Claude Code báo không tìm thấy
agent type, phiên hiện tại được mở trước khi hai file đó tồn tại — khởi động lại phiên.

## Bảo trì

Nâng cấp phần logic: `bash <workspace>/.claude/skills/ux-persona-lab/scripts/install.sh D:/DeleteByDuc/web-game/web-game-duck-defense --update`
